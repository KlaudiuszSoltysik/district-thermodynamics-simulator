...
class MeteringService:
    ...
    def __init__(self, num_nodes, district_id_dict):
        ...
        self.total_elec_import_kwh = 0.0
        self.total_elec_export_kwh = 0.0
        self.total_pv_yield_kwh = 0.0
        self.total_pv_self_consumed_kwh = 0.0
        self.total_gas_import_kwh = 0.0
    ...
    def update_meters(self, dt_seconds, energy_costs, q_hvac_w, v_vent_m3_s):
        ...
        self.admin_elec_cost += (grid_buy_kw * hours) * (
            energy_costs.electricity_price_eur_per_mwh / 1000.0
        )
        self.admin_gas_cost += (gas_buy_kw * hours) * (
            energy_costs.gas_price_eur_per_mwh / 1000.0
        )

        export_tariff = energy_costs.electricity_price_eur_per_mwh

        self.admin_elec_revenue += (grid_sell_kw * hours) * (export_tariff / 1000.0)
        ...
        for i in range(self.num_nodes):
            ...
            if power_w > 0:
                self.room_heat_delivered_kwh[room_id] += (power_w / 1000.0) * hours
                room_elec_kw += (power_w / energy_costs.cop_heating) / 1000.0
            elif power_w < 0:
                self.room_cool_delivered_kwh[room_id] += (abs(power_w) / 1000.0) * hours
                room_elec_kw += (abs(power_w) / energy_costs.cop_cooling) / 1000.0

            self.room_vent_volume_m3[room_id] += vent_m3_s * 3600.0 * hours
    ...
    def get_meter_readings(self):
        ...
        self_consumption_rate = (
            (self.total_pv_self_consumed_kwh / self.total_pv_yield_kwh * 100.0)
        ...
        self_sufficiency_rate = (
            (self.total_pv_self_consumed_kwh / total_building_usage_kwh * 100.0)
